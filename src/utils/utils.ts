...
export function stripMDExtensionFromPath(file: TFile): string | null {
...
export function generateMarkdownLink(
  ...
  sugg: AnySuggestion,
  sourcePath: string,
  options?: { useBasenameAsAlias: boolean | null; useHeadingAsAlias: boolean | null },
): string {
  let linkStr: string = '';
...
function sanitizeStringForLinkSubpath(
  input: string,
  useInputAsAlias: boolean | null,
): { subpath: string; alias?: string } {
  ...
  return {
    ...
    alias: useInputAsAlias ? sanitizedInput : undefined,
  };
}

/**
 * Determines if sugg is a file-based suggestion, and if so, returns the associated
 * destination TFile. NoteWise returns null.
...
function generateMarkdownLinkForReferenceCache(
  ...
  refCacheSourceFile: TFile | null,
  useBasenameAsAlias: boolean | null,
): string {
  const { link, displayText } = refCache;
  const { path, subpath } = parseLinktext(link);
  let alias = displayText;
  let destFile: TFile | null = null;
  let linkStr: string | null = null;
 ...
 * a type that matches viewType. NoteWise, false.
 *
 * @export
 * @param {WorkspaceLeaf} leaf
